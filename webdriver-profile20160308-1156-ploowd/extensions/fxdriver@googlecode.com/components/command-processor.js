...
wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING = {webStorageEnabled:"dom.storage.enabled", applicationCacheEnabled:"browser.cache.offline.enable", databaseEnabled:"dom.indexedDB.enabled", locationContextEnabled:"geo.enabled", browserConnectionEnabled:"dom.network.enabled", acceptSslCerts:"webdriver_accept_untrusted_certs", nativeEvents:"webdriver_enable_native_events", pageLoadingStrategy:"webdriver.load.strategy"};
      ...
      if (g = k.fxdriver) {
        if (k.getBrowser().contentWindow) {
          if (g.modalOpen && "getAlertText" != c.name && "setAlertValue" != c.name && "acceptAlert" != c.name && "dismissAlert" != c.name) {
            ...
            "function" != typeof g[c.name] && "function" != typeof WebElement[c.name] ? (d.sendError(new WebDriverError(bot.ErrorCode.UNKNOWN_COMMAND, "Unrecognised command: " + c.name)), goog.log.error(nsCommandProcessor.LOG_, "Unknown command: " + c.name)) : ("get" == c.name && d.session.setWaitForPageLoad(!1), d.startCommand(k), (new DelayedCommand(g, c, d)).execute(0));
          }
        } else {
